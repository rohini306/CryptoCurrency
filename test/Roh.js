...
it('transfers token ownership', function(){
  ...
  it('approves tokens for delegated transfer', function(){
    return Roh.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._owner,accounts[0],'logs the account the tokens are aurthorized by');
...
return tokenInstance.allowance(accounts[0],accounts[1]);
  ...
  assert.equal(allowance.toNumber(),100,'stores the allowance for the delegate transfer');
})
  })
...
it('handles delegate token transfer', function(){
  return Roh.deployed().then(function(instance){
    ...
    //approve spending Account to spend 10 tokens from from account
    return tokenInstance.approve(spendingAccount,10,{from:fromAccount});

  }).then(function(receipt){
    //try transferring something larger than the sender's balanceOf
    return tokenInstance.transferFrom(fromAccount,toAccount, 9999,{from:spendingAccount});
  }).then(assert.fail).catch(function(error){
    assert(error.message.indexOf('revert')>=0,'cannot transfer value larger than balances');
    //try transferring something larger than the approved accounts
...
return tokenInstance.balanceOf(fromAccount);
  ...
  assert.equal(balance.toNumber(),90,'deducts the amount from the sending amount');
